    ...
    toast((t) => (
      <div className="flex flex-col gap-3">
        <p className="font-bold text-sm">
          <span className="font-black">"{pkg.title}"</span> মুছে ফেলবেন?
        </p>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 py-2 bg-rose-500 hover:bg-rose-400 text-white text-xs font-black rounded-xl transition-colors"
          >
            হ্যাঁ, মুছুন
          </button>
          <button
            onClick={() => toast.dismiss(t.id)}
            className="flex-1 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 text-xs font-black rounded-xl transition-colors"
          >
            বাতিল
          </button>
        </div>
            ...
            <thead>
              <tr className="border-b border-slate-800">
                {["Package", "Destination", "Category", "Price", "Rating", "Featured", "Actions"].map((h) => (
                  <th key={h} className="text-left px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
